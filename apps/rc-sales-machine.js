...
var WIN=(1+0.28*wk('met'))*(1+0.19*wk('cul'))*(1+0.09*wkNarr());WIN=Math.min(WIN,1.6);
var convNew=convBase+(convBase*WIN-convBase)*LAMBDA;convNew=Math.min(convNew,convBase*3,60);
var VOL=LAMBDA*Math.min(0.50,wk('palancas')*0.30+wk('dep')*0.15+wk('cerebro')*0.10);
...
function build(dest){
 ...
 drawChips();calc();
 console.log('[rc] consola v4 montada · WIN='+WIN.toFixed(2)+' VOL='+VOL.toFixed(2));
}
