...
var convParts=CONV.map(function(l){var g,b;if(l.id==='narr'){g=wkNarr();b=100;}else{g=wk(l.id);b=BENCH[l.id];}var v=Math.round(b*(1-g));var bf=Math.max(0,Math.min(1,(b-v)/100));return {id:l.id,lab:l.lab,coef:l.coef,gap:g,bf:bf,pts:convBase*l.coef*bf,src:l.src,bench:b,v:v};});
