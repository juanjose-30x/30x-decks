(function(){
'use strict';
var CAL='https://calendly.com/d/cxpx-kmb-ccy/30x-sales-ai-sales-admisiones';
var ENDPOINT=window.SM_ENDPOINT||'https://diagnostico-leads-187963534040.us-central1.run.app/api/lead-sales-machine';
var PRICE=1950;
var CONSOLE='https://magnet.30x.com/caso-roi-ai-sales';
var SIG=[
 {id:'research',type:'pct',bench:70,imp:10,name:'Inteligencia de cuentas',q:'¿Qué % de tus cuentas objetivo trabajas con research/dossier hecho por IA antes del primer contacto?',up:'llegas al primer toque con el dossier de la cuenta ya armado, sin improvisar',pain:'Improvisas el primer toque; llegas sin contexto de la cuenta.',mod:'M4 · Inteligencia de cuentas'},
 {id:'outbound',type:'pct',bench:70,imp:11,name:'Outbound a escala',q:'¿Qué % de tu outbound sale personalizado a escala (no copy-paste manual)?',up:'pasas del mismo correo para todos a personalización 1:1 a escala',pain:'Mandas el mismo correo a todos; el pipeline no escala sin meter más gente.',mod:'M5 · Automatización a escala'},
 {id:'speed',type:'pct',bench:100,imp:12,name:'Speed-to-lead',q:'¿Qué % de tus leads reciben respuesta en menos de 5 minutos?',up:'dejas de perder al lead que otro contesta primero y respondes en segundos, 24/7',pain:'El lead se va con quien conteste primero; pierdes a los que no atiendes a tiempo.',mod:'M6 · Inbound por voz'},
 {id:'califica',type:'pct',bench:90,imp:9,name:'Calificación automática',q:'¿Qué % de tus leads se califican y enrutan sin intervención humana?',up:'dejas de quemar agenda en no-fits y el bot califica y agenda solo',pain:'Quemas agenda en no-fits que un bot podría filtrar solo.',mod:'M7 · Inbound por chat y redes'},
 {id:'coaching',type:'pct',bench:100,imp:8,name:'Inteligencia de conversación',q:'¿Qué % de tus llamadas de venta se analizan con IA (scorecard o patrones)?',up:'dejas de perder lo que se dijo y sacas patrones ganadores por deal',pain:'Se pierde lo que se dijo en la llamada; no sacas patrones para mejorar.',mod:'M8 · Conversational Intelligence'},
 {id:'crm',type:'pct',bench:50,imp:8,name:'Llenado de CRM automático',q:'¿Qué % de tu CRM se actualiza solo, sin que el equipo lo digite a mano?',up:'tu CRM se llena solo y dejas de perder data por digitación manual',pain:'El equipo pierde tiempo digitando el CRM y la data queda incompleta.',mod:'M9 · RevOps y automatización'},
 {id:'cerebro',type:'pct',bench:80,imp:9,name:'Cerebro de ventas con IA',q:'¿Qué % de tus procesos de venta están documentados en un cerebro/base de conocimiento de IA?',up:'tu operación queda documentada en un cerebro de IA que responde por ti',pain:'El conocimiento vive en la cabeza de cada quien, no en un sistema.',mod:'M10 · Segundo Cerebro de ventas'}
];
var ARQ=[
 {max:3,lvl:'Nivel 1',name:'A pulso',desc:'La venta depende de ti; la IA es, a lo sumo, ChatGPT suelto. Sin research, sin automatización, sin medición.',col:'#FF6B5A',bg:'rgba(255,107,90,.12)'},
 {max:6,lvl:'Nivel 2',name:'Herramientas sueltas',desc:'Usas varias herramientas de IA pero desconectadas del proceso. Wins aislados que no escalan.',col:'#E7C24B',bg:'rgba(231,194,75,.12)'},
 {max:8,lvl:'Nivel 3',name:'Centauro',desc:'IA integrada en research, outbound y análisis; parte del pipeline ya corre asistido sobre tu contexto.',col:'#EBFF6F',bg:'rgba(235,255,111,.12)'},
 {max:10,lvl:'Nivel 4',name:'Máquina que opera sin ti',desc:'Agentes autónomos sobre tu Segundo Cerebro: respuesta en segundos, calificación automática y CRM que se llena solo.',col:'#EBFF6F',bg:'rgba(235,255,111,.12)'}
];
var st={fit:null,touched:{},urgencia:null,sig:{research:50,outbound:50,speed:50,califica:50,coaching:50,crm:50,cerebro:50},siglab:{},conv:null,leads:0,narr:null,team:null,rev:0,willing:null,role:null,industry:null,nombre:'',empresa:'',email:'',whatsapp:''};
var CSS=[
...
function build(dest){
 ...
 SIG.forEach(function(g){var row=n('div',{cls:'sm-sig'});row.appendChild(n('div',{cls:'sm-sl-q',txt:g.q}));var sl=n('input',{type:'range',min:'0',max:'100',step:'1',value:String(st.sig[g.id]),'aria-label':g.name});var v=n('span',{cls:'sm-slv',txt:st.sig[g.id]+'%'});var b=n('div',{cls:'sm-bench'});function up(){var val=+sl.value;st.sig[g.id]=val;v.textContent=val+'%';}sl.addEventListener('input',function(){up();st.touched[g.id]=true;refresh();});row.appendChild(n('div',{cls:'sm-slrow'},[sl,v]));up();s2.appendChild(row);});steps.push(s2);
 ...
 var s4=n('section',{cls:'sm-step'});s4.appendChild(n('h2',{cls:'sm-q',txt:'Para dimensionar tu potencial: ¿cuánto factura tu negocio al mes?'}));s4.appendChild(n('p',{cls:'sm-help',txt:'Con tu facturación calculamos el tamaño de la oportunidad al integrar IA a tu proceso de ventas.'}));
 var revrow=n('div',{cls:'sm-sig'});revrow.appendChild(n('div',{cls:'sm-sl-q',txt:'¿Cuánto factura tu negocio al mes, aprox.? (USD)'}));var revin=n('input',{type:'tel',inputmode:'numeric',placeholder:'Ej: 40000'});revin.addEventListener('input',function(){st.rev=parseInt((revin.value||'').replace(/\D/g,''),10)||0;refresh();});revrow.appendChild(revin);s4.appendChild(revrow);var cvrow=n('div',{cls:'sm-sig'});cvrow.appendChild(n('div',{cls:'sm-sl-q',txt:'De cada 10 leads, ¿cuántas terminan en venta? (opcional)'}));var cvin=n('input',{type:'tel',inputmode:'numeric',placeholder:'Ej: 3 de 10'});cvin.addEventListener('input',function(){var x=parseInt((cvin.value||'').replace(/\D/g,''),10);st.conv=(isNaN(x)?null:Math.max(0,Math.min(10,x)));});cvrow.appendChild(cvin);s4.appendChild(cvrow);var ldrow=n('div',{cls:'sm-sig'});ldrow.appendChild(n('div',{cls:'sm-sl-q',txt:'¿Cuántos leads o prospectos nuevos entran al mes? (opcional)'}));var ldin=n('input',{type:'tel',inputmode:'numeric',placeholder:'Ej: 80'});ldin.addEventListener('input',function(){st.leads=parseInt((ldin.value||'').replace(/\D/g,''),10)||0;});ldrow.appendChild(ldin);s4.appendChild(ldrow);steps.push(s4);
 var s5=n('section',{cls:'sm-step'});s5.appendChild(n('h2',{cls:'sm-q',txt:'¿Cuál es tu rol frente a esto?'}));var orl=n('div',{cls:'sm-opts'});['dec|Founder / C-level (CEO, socio, GM)','dec|Líder de ventas (Head / Director / Gerente comercial)','dec|Rep / Growth / Ops (y yo lo implementaría)','no|Otro / sin mandato para decidir'].forEach(function(p){var a=p.split('|');orl.appendChild(opt(orl,a[0],a[1],function(v){st.role=v;refresh();}));});s5.appendChild(orl);steps.push(s5);
 ...
 function paintTop(nofit,sc,c){
  ...
  var head=n('div',{cls:'sm-head'},[ring,n('div',{},[bt,n('h3',{cls:'sm-q',style:'margin:10px 0 4px',txt:nofit?'Primero, activa tu proceso de ventas':a.name}),n('p',{cls:'sm-help',style:'margin:0',txt:nofit?'AI Sales es para negocios con venta 1:1 recurrente (no ecommerce/retail).':a.desc})])]);
  ...
  lock.appendChild(n('p',{cls:'sm-help',style:'margin:6px auto 0;max-width:420px;color:var(--a);font-weight:600',txt:'Con Nicolás Rojas (CEO de Dapta, Forbes 30U30) y Andrés Bilbao (co-founder de Rappi)'}));var f=n('div',{cls:'sm-lockform'});
 ...
 if(t==='low'){
  if(!nofit){
   var pot=n('div',{cls:'sm-pot'});pot.appendChild(n('div',{cls:'k',txt:'Potencial al integrar IA a tu venta'}));pot.appendChild(n('div',{cls:'v',txt:'+'+fmt(c.yr)+'/año'}));pot.appendChild(n('div',{cls:'s',txt:'≈ +'+fmt(c.mo)+'/mes sobre tu facturación actual.'}));
 ...
 pr.appendChild(n('div',{cls:'pk',txt:'30X · AI Sales · Diagnóstico'}));
 ...
 pr.appendChild(n('div',{cls:'psec',txt:'Qué trabajarías en AI Sales'}));
 ...
 pr.appendChild(n('div',{cls:'pfoot',txt:'El potencial es una estimación orientativa y conservadora sobre tu facturación, no una promesa de resultados. 30X · AI Sales · inicia 6 de octubre 2026.'}));
 document.body.appendChild(pr);
 holder.appendChild(n('p',{cls:'sm-foot',txt:'El potencial es una estimación orientativa y conservadora sobre tu facturación, no una promesa de resultados. 30X · AI Sales · inicia 6 de octubre 2026.'}));
