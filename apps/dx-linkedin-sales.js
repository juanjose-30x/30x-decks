(function(){
'use strict';
var CAL='https://calendly.com/d/d3xx-6qx-r8d/admisiones-linkedin-sales-30x';
var ENDPOINT=window.SM_ENDPOINT||'https://diagnostico-leads-187963534040.us-central1.run.app/api/lead-sales-machine';
var PRICE=1950;
var CONSOLE='https://magnet.30x.com/caso-roi-linkedin-sales';
var FREQ=[['Nunca',0],['Casi nunca',25],['A veces',50],['Casi siempre',75],['Siempre',100]];
var PUB=[['Nunca',0],['Esporádico',25],['Mensual',50],['Semanal',75],['Varias veces/semana',100]];
var CONEX=[['0',0],['1-5',25],['6-15',50],['16-30',75],['30+',100]];
var AUD=[['Menos de 500',10],['500-2.000',40],['2.000-10.000',75],['Más de 10.000',100]];
var CREC=[['Decrece',0],['Plano',33],['Crece lento',66],['Crece rápido',100]];
var SIG=[
 {id:'perfil',type:'pct',bench:90,imp:8,name:'Perfil-vitrina',q:'¿Qué % de tu perfil está optimizado para conversión (titular, banner, destacados, CTA)?',up:'pasas de un perfil-CV a una vitrina que tu cliente entiende al instante',pain:'Tu perfil es un CV; el decisor no entiende qué haces ni por qué escribirte.',mod:'M2 · Tu tienda (perfil)'},
 {id:'contenido',type:'freq',opts:PUB,bench:75,imp:10,name:'Sistema de contenido',q:'¿Con qué frecuencia publicas contenido en LinkedIn?',up:'dejas de publicar por impulso y pasas a un sistema de pilares que se repite solo',pain:'Publicas por impulso; no construyes autoridad que atraiga inbound.',mod:'M4-M6 · Pilares y contenido'},
 {id:'conexiones',type:'band',opts:CONEX,bench:75,imp:12,name:'Conexiones nuevas / semana',q:'¿Cuántas conexiones nuevas con personas relevantes (tu ICP) haces por semana?',up:'eliges tú a quién llegar cada semana y dejas de depender de referidos',pain:'Sumas pocas conexiones relevantes por semana; dependes de lo que llegue solo.',mod:'M8-M9 · ICP y Sales Navigator'},
 {id:'audiencia',type:'band',opts:AUD,bench:60,imp:8,name:'Audiencia',q:'¿Cuántos seguidores tienes hoy en LinkedIn?',up:'construyes una audiencia que te da autoridad e inbound',pain:'Tu audiencia es pequeña y no te da autoridad ni alcance.',mod:'M3 · Autoridad y marca'},
 {id:'crecimiento',type:'band',opts:CREC,bench:66,imp:8,name:'Crecimiento de audiencia',q:'¿Cómo ha crecido tu audiencia en los últimos meses?',up:'tu audiencia crece de forma sostenida, no plana',pain:'Tu audiencia está plana; no crece mes a mes.',mod:'M4-M6 · Contenido que crece'},
 {id:'mensajes',type:'freq',opts:FREQ,bench:75,imp:11,name:'Mensajes que abren conversación',q:'¿Con qué frecuencia tus mensajes de conexión abren una conversación real (no spam)?',up:'dejas de sonar a spam y abres conversaciones con tu cliente ideal',pain:'Tus mensajes suenan a spam y casi nadie responde.',mod:'M10 · Mensajes que responden'},
 {id:'ventas',type:'pct',bench:50,imp:10,name:'Ventas desde LinkedIn',q:'¿Qué % de tus ventas o reuniones B2B vienen de LinkedIn?',up:'LinkedIn se vuelve un canal real de pipeline, medible en un dashboard',pain:'LinkedIn casi no te trae ventas ni reuniones B2B hoy.',mod:'M12 · Dashboard y optimización'}
];
var ARQ=[
 {max:3,lvl:'Nivel 1',name:'Invisible',desc:'Perfil-CV, no publicas, dependes de referidos y voz a voz. LinkedIn es una hoja de vida parada.',col:'#FF6B5A',bg:'rgba(255,107,90,.12)'},
 {max:6,lvl:'Nivel 2',name:'Presente sin sistema',desc:'Perfil decente y publicas a ratos, pero prospectas en frío sin autoridad ni método.',col:'#E7C24B',bg:'rgba(231,194,75,.12)'},
 {max:8,lvl:'Nivel 3',name:'Autoridad en construcción',desc:'Perfil-vitrina, contenido consistente y prospección con ICP; empiezan a llegar conversaciones reales.',col:'#EBFF6F',bg:'rgba(235,255,111,.12)'},
 {max:10,lvl:'Nivel 4',name:'Máquina de pipeline',desc:'Autoridad que genera inbound + outbound sistematizado + dashboard; reuniones B2B predecibles.',col:'#EBFF6F',bg:'rgba(235,255,111,.12)'}
];
var st={fit:null,touched:{},urgencia:null,sig:{perfil:50,contenido:0,conexiones:0,audiencia:0,crecimiento:0,mensajes:0,ventas:50},siglab:{},conv:null,leads:0,narr:null,team:null,rev:0,willing:null,role:null,industry:null,nombre:'',empresa:'',email:'',whatsapp:''};
var CSS=[
...
function score(){var s=0;SIG.forEach(function(g){s+=pts(st.sig[g.id],g.bench);});return Math.round(s/(SIG.length*2)*10);}
function arqOf(sc){for(var i=0;i<ARQ.length;i++){if(sc<=ARQ[i].max)return ARQ[i];}return ARQ[ARQ.length-1];}
var REV_MIN=5000,REV_HIGH=15000;
...
function build(dest){
 ...
 var s3=n('section',{cls:'sm-step'});s3.appendChild(n('h2',{cls:'sm-q',txt:'Una cosa más sobre tu equipo'}));
 s3.appendChild(n('div',{cls:'sm-sl-q',txt:'¿A cuántas personas diriges en el área de ventas?',style:'margin-top:6px'}));var ot=n('div',{cls:'sm-opts'});['mas5|Más de 5: dirijo un equipo comercial','2a5|Entre 2 y 5','1|1 persona','solo|Ninguna, vendo yo mismo'].forEach(function(p){var a=p.split('|');ot.appendChild(opt(ot,a[0],a[1],function(v){st.team=v;refresh();}));});s3.appendChild(ot);s3.appendChild(n('div',{cls:'sm-sl-q',txt:'¿Cuándo quieres resolver esto?',style:'margin-top:18px'}));var ou=n('div',{cls:'sm-opts'});['ya|Ahora: es prioridad este trimestre','pronto|En los próximos 3-6 meses','explora|Solo estoy explorando'].forEach(function(p){var a=p.split('|');ou.appendChild(opt(ou,a[0],a[1],function(v){st.urgencia=v;refresh();}));});s3.appendChild(ou);steps.push(s3);
 var s4=n('section',{cls:'sm-step'});s4.appendChild(n('h2',{cls:'sm-q',txt:'Para dimensionar tu potencial: ¿cuánto generas hoy al mes?'}));s4.appendChild(n('p',{cls:'sm-help',txt:'Con lo que generas hoy (o esperas generar) calculamos el tamaño de la oportunidad al montar tu pipeline en LinkedIn.'}));
 var revrow=n('div',{cls:'sm-sig'});revrow.appendChild(n('div',{cls:'sm-sl-q',txt:'¿Cuánto generas al mes con tus servicios, o lo que esperas generar? (USD)'}));var revin=n('input',{type:'tel',inputmode:'numeric',placeholder:'Ej: 40000'});revin.addEventListener('input',function(){st.rev=parseInt((revin.value||'').replace(/\D/g,''),10)||0;refresh();});revrow.appendChild(revin);s4.appendChild(revrow);var cvrow=n('div',{cls:'sm-sig'});cvrow.appendChild(n('div',{cls:'sm-sl-q',txt:'De cada 10 leads o contactos, ¿cuántas terminan en venta? (opcional)'}));var cvin=n('input',{type:'tel',inputmode:'numeric',placeholder:'Ej: 3 de 10'});cvin.addEventListener('input',function(){var x=parseInt((cvin.value||'').replace(/\D/g,''),10);st.conv=(isNaN(x)?null:Math.max(0,Math.min(10,x)));});cvrow.appendChild(cvin);s4.appendChild(cvrow);var ldrow=n('div',{cls:'sm-sig'});ldrow.appendChild(n('div',{cls:'sm-sl-q',txt:'¿Cuántos leads o prospectos nuevos entran al mes? (opcional)'}));var ldin=n('input',{type:'tel',inputmode:'numeric',placeholder:'Ej: 80'});ldin.addEventListener('input',function(){st.leads=parseInt((ldin.value||'').replace(/\D/g,''),10)||0;});ldrow.appendChild(ldin);s4.appendChild(ldrow);steps.push(s4);
 var s5=n('section',{cls:'sm-step'});s5.appendChild(n('h2',{cls:'sm-q',txt:'¿Cuál es tu rol frente a esto?'}));var orl=n('div',{cls:'sm-opts'});['dec|Founder / C-level','dec|Líder de ventas / comercial','solo|Consultor / fractional / vendo mis propios servicios con mi marca','no|Empleado sin mandato / otro'].forEach(function(p){var a=p.split('|');orl.appendChild(opt(orl,a[0],a[1],function(v){st.role=v;refresh();}));});s5.appendChild(orl);steps.push(s5);
 ...
 function refresh(){var ok=true;if(i===0)ok=!!st.fit&&(st.fit==='no'||!!st.industry);if(i===1)ok=(Object.keys(st.touched).length>=SIG.length);if(i===2)ok=(st.team!==null&&st.urgencia!==null);if(i===3)ok=st.rev>0;if(i===4)ok=!!st.role;next.disabled=!ok;}
 function valid(){if(i===0&&!st.fit)return 'Elige una opción.';if(i===0&&st.fit!=='no'&&!st.industry)return 'Elige la opción que describe tu negocio.';if(i===1&&Object.keys(st.touched).length<SIG.length)return 'Mueve cada control para calibrar tu diagnóstico.';if(i===2&&(st.team===null||st.urgencia===null))return 'Responde ambas.';if(i===3&&!(st.rev>0))return 'Escribe cuánto generas (solo números).';if(i===4&&!st.role)return 'Elige tu rol.';return '';}
 ...
 function paintTop(nofit,sc,c){
  ...
  var head=n('div',{cls:'sm-head'},[ring,n('div',{},[bt,n('h3',{cls:'sm-q',style:'margin:10px 0 4px',txt:nofit?'Primero, activa tu proceso de ventas':a.name}),n('p',{cls:'sm-help',style:'margin:0',txt:nofit?'LinkedIn Sales es para negocios con venta 1:1 recurrente (no ecommerce/retail).':a.desc})])]);
  ...
  lock.appendChild(n('p',{cls:'sm-help',style:'margin:6px auto 0;max-width:420px;color:var(--a);font-weight:600',txt:'Con Daniela Luque · Top 3 creadores hispanos en LinkedIn · +200 ejecutivos formados'}));var f=n('div',{cls:'sm-lockform'});
 ...
 if(t==='low'){
  if(!nofit){
   var pot=n('div',{cls:'sm-pot'});pot.appendChild(n('div',{cls:'k',txt:'Potencial con tu máquina de pipeline'}));pot.appendChild(n('div',{cls:'v',txt:'+'+fmt(c.yr)+'/año'}));pot.appendChild(n('div',{cls:'s',txt:'≈ +'+fmt(c.mo)+'/mes sobre tu facturación actual.'}));
 ...
 pr.appendChild(n('div',{cls:'pk',txt:'30X · LinkedIn Sales · Diagnóstico'}));
 ...
 pr.appendChild(n('div',{cls:'psec',txt:'Qué trabajarías en LinkedIn Sales'}));
 ...
 pr.appendChild(n('div',{cls:'pfoot',txt:'El potencial es una estimación orientativa y conservadora sobre tu facturación, no una promesa de resultados. 30X · LinkedIn Sales · inicia 6 de octubre 2026.'}));
 document.body.appendChild(pr);
 holder.appendChild(n('p',{cls:'sm-foot',txt:'El potencial es una estimación orientativa y conservadora sobre tu facturación, no una promesa de resultados. 30X · LinkedIn Sales · inicia 6 de octubre 2026.'}));
