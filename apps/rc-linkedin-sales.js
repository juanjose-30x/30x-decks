...
var DECK=window.DECK_URL||'https://juanjose-30x.github.io/30x-decks/LinkedIn-Sales-30X.pdf';
var BENCH={perfil:90,contenido:75,conexiones:75,audiencia:60,crecimiento:66,mensajes:75,ventas:50};
var PAINS=[
 {id:'perfil',lab:'Perfil-CV',line:'Tu perfil es un CV. Convertirlo en vitrina hace que el decisor entienda qué haces y te escriba.'},
 {id:'contenido',lab:'Publica por impulso',line:'Publicas por impulso. Un sistema de pilares te vuelve referente y atrae inbound solo.'},
 {id:'conexiones',lab:'Pocas conexiones nuevas',line:'Sumas pocas conexiones relevantes por semana. Con una cadencia de prospección eliges tú a quién llegar y llenas el pipeline.'},
 {id:'audiencia',lab:'Audiencia pequeña',line:'Tu audiencia es chica y no te da autoridad. Creciéndola, empieza a llegarte inbound solo.'},
 {id:'crecimiento',lab:'Audiencia estancada',line:'Tu audiencia está plana. Con contenido consistente vuelve a crecer y con ella tu alcance comercial.'},
 {id:'mensajes',lab:'Mensajes tipo spam',line:'Tus mensajes suenan a spam y nadie responde. Con secuencias que abren conversación llenas la agenda.'},
 {id:'ventas',lab:'LinkedIn no vende',line:'Hoy LinkedIn casi no te trae ventas. Sistematizado, se vuelve un canal medible de reuniones B2B.'}
];
var Q=new URLSearchParams(location.search);
var sel={};PAINS.forEach(function(p){sel[p.id]=false;});
var dx=(Q.get('d')||'').split(',').map(function(s){return s.trim();}).filter(Boolean);
if(dx.length){dx.forEach(function(id){if(sel.hasOwnProperty(id))sel[id]=true;});}else{sel.conexiones=true;sel.mensajes=true;sel.contenido=true;}
...
var PIPE=[{id:'conexiones',lab:'Cadencia de prospecci\u00f3n',coef:0.30,src:'LinkedIn: l\u00edderes de social selling generan +45% oportunidades'},{id:'contenido',lab:'Contenido por pilares',coef:0.15,src:'HubSpot: 56% de vendedores usan redes para hallar prospectos'},{id:'audiencia',lab:'Crecer la audiencia',coef:0.10,src:'M\u00e1s audiencia = m\u00e1s alcance comercial e inbound'},{id:'crecimiento',lab:'Audiencia consistente',coef:0.05,src:'Publicar consistente reactiva el crecimiento del alcance'}];
...
var cb=n('div',{cls:'rc-src'});
 ...
 cb.appendChild(n('p',{cls:'rc-scn',style:'margin:0 0 4px',html:'Tu conversión hoy es <b>'+Math.round(convBase)+'%</b> y con LinkedIn como canal llegaría a <b>'+sugTgt+'%</b>. Así aporta cada palanca, según tu diagnóstico:'}));
